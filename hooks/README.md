...
| `scripts/flutter_env.py` | Runs an approved Flutter hook task with `GIT_DIR`/`GIT_WORK_TREE` cleared from the child environment, so Flutter resolves its own SDK checkout in linked git worktrees (where `git commit` exports those vars into hook env). Portable via `language: python`. |
