...
| Moving work between agents or IDEs | Optional — prefer `TO_DO.md`, `plans/active/`, and `.context/project-profile.md`; if you use [`templates/handoff.md`](templates/handoff.md), keep `.context/handoff.md` date-stamped and current (a stale handoff is worse than none) |
...
- Handoffs are optional. Prefer durable trackers (`TO_DO.md`, `plans/active/`,
  ...
  whenever you pause or switch agents/IDEs, and treat one whose date predates the
  current work as stale — re-derive state instead of trusting it. Delete it once the
  work lands.
...
- Use a gitignored in-repo scratch directory (`tmp/` or `temp/` — both are in
  ...
  scratch files outside the repository (`/tmp`, `/var/folders`, or any other external
  temp path): every external write triggers a per-command permission prompt and slows
  the workflow, and absolute `/var/folders` paths are flagged by
  `hooks/scripts/check_absolute_paths.sh`. If the scratch dir is missing, create it and
