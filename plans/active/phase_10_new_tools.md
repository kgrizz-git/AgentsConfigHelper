...
- [ ] **Secret Handling Policy:** Define a security policy for secret-bearing files (e.g., Kilo's `models.json`). Backups of secret-bearing project files must be routed to the app's secure global backup directory rather than writing a `.bak` file into the project directory, to guarantee they are never accidentally committed. Add regression tests for this behavior.
