...
Date: 2026-08-23
Author: maintainers
Status: ready to implement — read-only presentation only
Linked issue/PR: n/a
...
The adapter applies only to the `claudeCode` descriptor's user and project
`.claude/settings.json` targets. It recognizes this exact `permissions` subtree:
...
- [ ] Read the linked Claude documentation at implementation time and record any schema
      version/behavior caveat in `docs/supported-tools.md` only if the curated reference
      needs correction.
- [ ] Add token-free fixtures for: complete policy, omitted optional fields, extra unknown
...
- [ ] Add a pure-Dart adapter test for every fixture. Assert both extracted values and the
      explicit eligibility/fallback reason—never a silent null result.
- [ ] Assert parsing itself preserves `ToolConfig.originalContent` and `rawSettings` for
      all fixtures.

### Phase 1 — read-only presentation

- [ ] Add an immutable presentation model for default mode, policy groups, unknown-key
      notice, help text, and documentation URI.
- [ ] Select the Claude adapter from the discovered descriptor identity. Manual files that
      are not confidently classified as Claude Code must not receive the card.
- [ ] Build a read-only card with distinguishable Allow/Ask/Deny labels, empty-state text,
      rule counts, and sensible wrapping/scrolling for long rule strings.
- [ ] Make help keyboard-accessible and expose the official documentation URI without
      forcing the app to open a browser in a widget test.
- [ ] Ensure the current “nested permissions … not editable” text is replaced only for a
...
- [ ] Do not add controls that mutate permission values, and do not change
      `JsonConfigParser.serialize` in this phase.

### Phase 2 — regression coverage and manual check

- [ ] Unit-test selection, recognized extraction, malformed fallback, unknown-key notice,
      and non-Claude rejection.
- [ ] Widget-test complete, empty, malformed, unknown-key, and raw-editor fallback states;
      assert rule text is readable rather than relying only on snapshot structure.
- [ ] Test an unchanged-card interaction performs no save and leaves exact raw content
