...
- **`.context/handoff.md`** — an *optional* packet from the previous agent/IDE/session. Check its
  **Date** first: if it predates the latest commit (`git log -1`), treat it as stale — re-derive
  state from `TO_DO.md`, `plans/active/`, and the working tree, then update or delete the file
  instead of trusting it. When current, read its **Next action** first and note what it lists as
  verified vs assumed. Delete it once the work lands.
