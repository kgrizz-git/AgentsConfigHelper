...
| User models cache | `~/.config/kilo/models.json` | registered (optional; not vendor-documented, see note) |
...
> present. Kilo's current documentation lists no `models.json` in the config directory
> (its model catalog is an in-memory cache), so the registered `models.json` target is
> labeled optional/legacy and must not imply a broken installation. The runtime config's
