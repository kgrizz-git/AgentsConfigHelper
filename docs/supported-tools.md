...
- **Read-only card:** The app renders the permission block of a discovered `config.toml` as a read-only policy card showing the file's stored entries; it does not compute Codex's effective policy and cannot edit TOML structure.
...
> **JSONC:** Cursor accepts JSONC in `permissions.json` (comments and trailing commas)
> even though the filename ends in `.json`. This app parses such a file via its JSONC
> fallback, and the formatting-fidelity notice follows the parsed content (labeling it
> "JSONC") rather than the `.json` filename. The app renders this file as a read-only
> policy card showing its stored entries; it does not compute Cursor's effective policy
> (user and project files are combined, and team-admin or in-app settings can take
> precedence).
...
| Scope | Path | Discovered? |
| --- | --- | --- |
| User settings (primary) | `~/.config/kilo/kilo.jsonc` | registered |
| User settings (alternate) | `~/.config/kilo/kilo.json` | registered (optional) |
| Model catalog cache | `~/.cache/kilo/models.json` | documented, not registered (cache, not config) |
| User rules | `~/.config/kilo/AGENTS.md` | registered |
| User agents | `~/.config/kilo/agents/*.md` | registered |
| Project settings (primary) | `kilo.jsonc` in project root | registered |
| Project settings (alternate) | `kilo.json` in project root | registered (optional) |
| Project settings (alt directory) | `.kilo/kilo.jsonc` / `.kilo/kilo.json` (takes priority when present) | registered (optional) |
...
> `kilo.jsonc`/`kilo.json`, `AGENTS.md`, and `agents/*.md` targets above
> are registered in `lib/catalog/tool_descriptor_registry.dart`; the remaining rows are
> retained as vendor-documented references. `kilo.json` is a supported alternate to the
> primary `kilo.jsonc`, and `.kilo/kilo.jsonc` takes priority over the root file when
> present. Kilo's model catalog is a cache at `~/.cache/kilo/models.json`, inherited from
> OpenCode's models.dev cache; it is machine-generated, not user-editable config, and is
> intentionally not a catalog target. The runtime config's
